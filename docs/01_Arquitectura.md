# Reproducción — Arquitectura del microservicio

## Componentes

| Componente | Rol |
|---|---|
| `ReproduccionApplication` | Punto de entrada Spring Boot |
| `controller/ReproduccionController.java` | API REST expuesta en `/api/reproducciones` |
| `service/ReproduccionService.java` | Lógica de negocio y transacciones |
| `repository/ReproduccionRepository.java` | Acceso JPA a H2 |
| `model/Reproduccion.java` | Entidad persistida en tablas H2 |
...
`Cliente/Sistema → API Gateway → ReproduccionController → ReproduccionService → ReproduccionRepository → H2`
