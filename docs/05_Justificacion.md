# ms-reproduccion — Justificación del servicio y cobertura de requisitos

**Caso caso03 — StreamVerse** (Plataforma de streaming) · EP01

Este documento justifica la existencia de **ms-reproduccion** como microservicio independiente: qué requisitos del negocio cubre (funcionales, no funcionales y de seguridad), por qué está delimitado así (SRP), y qué tecnología AWS se usa para cada responsabilidad y **por qué**. Los diagramas que respaldan esta justificación están en `docs/diagramas/`.

---

## 1. Misión del servicio

ms-reproduccion mantiente el seguimiento en tiempo real del proceso: consume los eventos y señales del dominio (estados, posición, sensores, etapas), mantiene el estado vigente consultable en milisegundos y notifica las transiciones del caso caso03 (StreamVerse).
...
| RF | Requisito (de `00_PresentacionEmpresa.md`) | Qué hace ms-reproduccion al respecto | Evidencia |
|----|------------------------------------------|-------------------------------|-----------|
| **RF-03** | Reproducir video y música con calidad adaptativa según la conexión | Ingresa y consulta el estado en tiempo real: cada evento actualiza el recurso y dispara la notificación correspondiente | diagrama de secuencia (evento en tiempo real) |
| **RF-04** | Registrar el progreso de reproducción (continue watching) | Ingresa y consulta el estado en tiempo real: cada evento actualiza el recurso y dispara la notificación correspondiente | diagrama de secuencia (evento en tiempo real) |
...
| RNF | Criterio | Cómo lo cumple ms-reproduccion | Decisión técnica |
|-----|----------|--------------------------|------------------|
| **RNF-05** (Rendimiento) | Tiempo de inicio de reproducción menor a 2 segundos; catálogo con baja latencia | Respuestas dentro del umbral exigido por el caso (ver alarmas p95) | Caché/bajo acoplamiento + CloudWatch con alarma de latencia |
| **RNF-01** (Escalabilidad) | Escalar reproducción y catálogo horizontalmente ante millones de usuarios simultáneos | Auto scaling independiente de este servicio (2→20 tareas Fargate según carga) | ECS Fargate + alarmas de CloudWatch: solo este componente escala en el pico |

**Justificación SRP (IE9):** ms-reproduccion tiene **una sola razón de cambio**: las reglas de seguimiento y transición de estados del proceso en tiempo real. Si mañana cambia esa regla, **ningún otro servicio se modifica**.
...
| Tecnología | Para qué se usa en ms-reproduccion |
|------------|------------------------------|
...
| Servicio AWS | Rol en ms-reproduccion | Por qué se eligió |
|--------------|----------------|--------------------|
...
## 6. Delimitación: qué NO hace ms-reproduccion (IE9/IE10)
...
| recomendaciones | ms-recomendaciones | razones de cambio distintas: los reportes viven aquí, pero los datos los generan los servicios operacionales |
...
├── secuencia/
│   └── Secuencia-Reproduccion   evento en tiempo real
└── infraestructura/
