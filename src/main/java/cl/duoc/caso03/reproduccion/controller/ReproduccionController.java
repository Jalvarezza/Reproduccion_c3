package cl.duoc.caso03.reproduccion.controller;
...
public class ReproduccionController {
    ...
    @GetMapping
    public ResponseEntity<List<Reproduccion>> listar() {
        return ResponseEntity.ok(service.findAll());
    }

    @GetMapping("/{id}")
    public ResponseEntity<Reproduccion> obtener(@PathVariable Long id) {
        return service.findById(id).map(ResponseEntity::ok)
                .orElse(ResponseEntity.notFound().build());
    }

    @PostMapping
    public ResponseEntity<Reproduccion> crear(@Valid @RequestBody Reproduccion recurso) {
        return ResponseEntity.status(HttpStatus.CREATED).body(service.create(recurso));
    }

    @PutMapping("/{id}")
    public ResponseEntity<Reproduccion> actualizar(@PathVariable Long id,
            @Valid @RequestBody Reproduccion datos) {
