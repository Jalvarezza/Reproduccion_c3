...
class ReproduccionControllerTest {
    ...
    private Reproduccion recurso(Long id) {
    ...
    void obtenerInexistenteDevuelve404() throws Exception {
        ...
        mockMvc.perform(get("/api/reproducciones/1")).andExpect(status().isNotFound());
    }
    ...
    void actualizarExistenteDevuelve200() throws Exception {
        Reproduccion actualizado = recurso(1L);
    ...
    void eliminarExistenteDevuelve204() throws Exception {
        ...
        mockMvc.perform(delete("/api/reproducciones/1")).andExpect(status().isNoContent());
    }

    @Test
    void eliminarInexistenteDevuelve404() throws Exception {
        when(service.delete(1L)).thenReturn(false);
        mockMvc.perform(delete("/api/reproducciones/1")).andExpect(status().isNotFound());
    }
}
