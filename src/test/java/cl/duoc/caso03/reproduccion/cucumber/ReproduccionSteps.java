package cl.duoc.caso03.reproduccion.cucumber;
...
public class ReproduccionSteps {
    ...
    private Long id;
    private ResponseEntity<Reproduccion> respuesta;
    private ResponseEntity<Void> respuestaVoid;

    private String url() {
        return "http://localhost:" + port + "/api/reproducciones";
    }
    ...
    public void servicioDisponible(String servicio) {
        ResponseEntity<Reproduccion[]> listado = rest.getForEntity(url(), Reproduccion[].class);
    ...
    public void crearRecurso(String recurso, String nombre) {
        ResponseEntity<Reproduccion> creado = rest.postForEntity(
                url(), body(nombre), Reproduccion.class);
    ...
    public void consultarCreado(String recurso) {
        respuesta = rest.getForEntity(url() + "/" + id, Reproduccion.class);
    }
    ...
    public void actualizar(String recurso, String nombre) {
        respuesta = rest.exchange(url() + "/" + id, HttpMethod.PUT, body(nombre), Reproduccion.class);
    }
    ...
    public void consultarEliminado(String recurso, int codigo) {
        ResponseEntity<Reproduccion> noExiste = rest.getForEntity(url() + "/" + id, Reproduccion.class);
